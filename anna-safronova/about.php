<?php 
  $page_title = "About | UIBrush Agency";
  include 'inc/header.php';
?>

<!-- ======== slider ========= -->
<section id="about-slider" class="slider">
	<div class="contain">
		<h1>Who <span class="highlight">We Are</span></h1>
		<p>Et harum quidem rerum facilis est et expedita distinctio. Nam libero tempore, cum soluta nobis est eligendi optio 
		nihil impedit quo facilis est et expedita distinctio minus id quod maxime placeat facere possimus, omnis voluptas assumenda est.</p>	
	</div><!-- /contain -->
</section>

<!-- ======== about ========= -->
<section id="about">
	<div class="contain">
		<h1 class="heading">About Us</h1>
		<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum.  Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum. Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum.  
	</p>
	</div><!-- /contain -->
</section>

<!-- ======== team ========= -->
<section id="team">
	<div class="contain clearfix">
		<h1 class="heading">Our Team</h1>
		<div class="cards">
			<div class="member-card">
				<img src="img/member-1.jpg" alt="Treadwell's image">
				<h5>Treadwell</h5>
				<p>Finacial Manager</p>

				<h6>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. 
				</h6>
			</div>

			<div class="member-card">
				<img src="img/member-1.jpg" alt="Elzey's image">
				<h5>Elzey</h5>
				<p>Project Manager</p>

				<h6>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. 
				</h6>
			</div>

			<div class="member-card">
				<img src="img/member-1.jpg" alt="Neil's image">
				<h5>Neil</h5>
				<p>Marketing Executive</p>

				<h6>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. 
				</h6>
			</div>

			<div class="member-card">
				<img src="img/member-1.jpg" alt="Maes's image">
				<h5>Maes</h5>
				<p>CEO</p>

				<h6>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. 
				</h6>
			</div><!-- /member-card -->
		</div><!-- /cards -->
	</div><!-- /contain -->
</section>

<!-- ======== mission ========= -->
<section id="mission">
	<div class="contain clearfix">
		<div class="text">	
			<h1 class="heading">Our Mission</h1>
			<h6>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum.</h6>
			<h6>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum.</h6>
			<h6>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor  ut labore eturet dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum.</h6>
		</div><!-- /text -->
		<img src="img/mission.jpg" alt="Mission image">
	</div><!-- /contain -->
</section>

<?php include "inc/footer.php"; ?> 